...
{
    /// <summary>
    /// Czytanie narzędzi z pliku NC
    /// </summary>
    public class ToolNcService : IToolService
    {
        private static List<Tool> _toolsFromNc = new List<Tool>();
        private static NcMainProgramService _ncMainProgramService = new NcMainProgramService();
        private static ToolsXmlFile _toolsXmlFile = new ToolsXmlFile();
        public int count = 0;
        ...
        {
            var fileList = new FileService();
            var _toolsFromNc = new List<Tool>() { };
            if (File.Exists(mainProgramFile))
            {
                var listsuprogramms = fileList.GetSubprogramsListFromNc(mainProgramFile).ToList();
        ...
        private bool checkIfIsMainProgram(string file)
        {
            bool result = false;
            var nc = GetNcLinesFromNC(file);
            var valid = nc.Count(nc => nc.Line.Contains("L9006"));
            if (valid > 1)
            {
                result = true;
                return result;
        ...
        {
            string program = _ncMainProgramService.GetMainProgramMvc();
            _toolsFromNc.Clear();
            _toolsFromNc = LoadToolsFromFile(program);
            return _toolsFromNc;
        }

        public List<Tool> GetAllToolsFromCurrentXml()
        {
            var mainProgam = _toolsXmlFile.GetMainProgramFileFromCurrentToolsXml();
            _toolsFromNc.Clear();
            _toolsFromNc = LoadToolsFromFile(mainProgam);
            return _toolsFromNc;
        }

        public Tool GetById(int id)
        {
            return _toolsFromNc.Single(m => m.Id == id);
        }
        public void Delete(int id)
        {
            var toolNc = GetById(id);
            _toolsFromNc.Remove(toolNc);
        }
    }
}
